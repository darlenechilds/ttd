 # ==========================================================
# Function: sf6_plot_conc.R
#
# Project:  ttd
# Purpose: plot conc for the timeseries for each water mass for the LSWcentre
#       surf = 1-200 dbar   
...
# ==========================================================
 ...
 sf6_plot_conc <- function(sf6){
   ...
   surf_ave <- aggregate(SF6 ~ yr, data = surf, FUN = mean)
   lsw_int_ave <- aggregate(SF6 ~ yr, data = lsw_int, FUN = mean)
   neadw_ave <- aggregate(SF6 ~ yr, data = neadw, FUN = mean)
   dsow_ave <- aggregate(SF6 ~ yr, data = dsow, FUN = mean)
   
   #plot each water mass
   plot(surf_ave$yr,surf_ave$SF6, ylim = c(0, 4), pch = 16, type = "b", xlab = "Year", ylab = "SF6 (fmol kg-1)")
   points(lsw_int_ave$yr,lsw_int_ave$SF6, col = "blue", pch = 16, type = "b")
   points(neadw_ave$yr,neadw_ave$SF6, col = "darkgreen", pch = 16, type = "b")
   points(dsow_ave$yr,dsow_ave$SF6, col = "darkred", pch = 16, type = "b")
