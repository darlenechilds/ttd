...
# ==========================================================
 ...
 f12_plot_conc <- function(f12){
   
     
   # get lsw centre
   d <- f12[f12$LATITUDE > 56 & f12$LATITUDE < 60,]
   ...
   surf_ave <- aggregate(CFC12 ~ yr, data = surf, FUN = mean)
   lsw_int_ave <- aggregate(CFC12 ~ yr, data = lsw_int, FUN = mean)
   neadw_ave <- aggregate(CFC12 ~ yr, data = neadw, FUN = mean)
   dsow_ave <- aggregate(CFC12 ~ yr, data = dsow, FUN = mean)
   
   #plot each water mass
   plot(surf_ave$yr,surf_ave$CFC12, ylim = c(0, 4), pch = 16, type = "b", xlab = "Year", ylab = "CFC12 (pmol kg-1")
   points(lsw_int_ave$yr,lsw_int_ave$CFC12, col = "blue", pch = 16, type = "b")
   points(neadw_ave$yr,neadw_ave$CFC12, col = "darkgreen", pch = 16, type = "b")
   points(dsow_ave$yr,dsow_ave$CFC12, col = "darkred", pch = 16, type = "b")
   legend("bottomright",
