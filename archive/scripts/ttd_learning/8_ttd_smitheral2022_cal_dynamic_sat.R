...
d <- data.frame(year = 2015:2025, mld = c(1700, 1850, 1900, 2000, 1400, 1500, 800, 1600, 800, 600, 600), 
                  sat = c(82.36, 80.52, NA, 80.74, 79.23, 81.04, NA, 76.45, 87.43, 84.175, 77.47),
                  dCdt = c(0.32, 0.34,0.35, 0.35, 0.33, 0.34, 0.39, 0.37, 0.4, 0.4, 0.4))

mlr <- lm(sat ~ dCdt + mld, data = d)
a <- mlr$coefficients[1]
b <- mlr$coefficients[2]
c <- mlr$coefficients[3]

sf6_atm <- read.csv("E:/A_docs/2025/2025_labSea/reports/ventialation_labSea_2015_2025/ttd_learning/atmos_data_sf6_comb_wTS.csv")
sf6_atm$dCdt <- c(NA, diff(sf6_atm$SF6NH))  # NA for first year  dC/dt = (Catm(t) - Catm (t-1))/1

mld <- read.csv("E:/A_docs/2025/2025_labSea/reports/ventialation_labSea_2015_2025/ttd_learning/Raimondi_MLD_S2.csv", header = F)
colnames(mld) <- c("Year","mld")

# Use approx to interpolate MLD to main$year
sf6_atm$MLD <- approx(x = mld$Year, y = mld$mld, xout = sf6_atm$Year, rule = 2)$y
sf6_atm$sat <- a + b*sf6_atm$dCdt + c*sf6_atm$MLD

write.csv(sf6_atm,"atmos_data_sf6_comb_wTS_wSat.csv", row.names = F)
...
# sf6_atm$sf6_atm_eff <- (sf6_atm$sf6_atm_atm*80)/100
...
write.csv(results,"TTD_sf6_Glodap_2.csv", row.names = F)
dir()
