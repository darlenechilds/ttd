...
obs_data <- read.csv("E:/A_docs/2025/2025_labSea/reports/ventialation_labSea_2015_2025/data/data_raw_shortcuts/2015_2025_lab_tracers.csv")  
names(obs_data)

# Convert the date column to year

# Remove leading/trailing whitespace and quotes
cleaned <- trimws(gsub('"', '', obs_data$date))

# Parsing using multiple formats and extract year
library(lubridate)
obs_data$years <- ifelse(grepl("^[A-Za-z]", cleaned),
                year(parse_date_time(cleaned, orders = "b d Y")),
                year(parse_date_time(cleaned, orders = c("d-m-Y", "d/m/Y"))))


# Define TTD, compute_Cxt, and solubility logic first (already done in your script)
# Make sure your temperature and salinity are dynamically used in the solubility calc

obs_conc <- obs_data$sf6_fmolperkg
t_obs <- obs_data$year
...
results <- do.call(rbind, lapply(1:nrow(obs_data), function(i) {
  row <- obs_data[i, ]
  fit_result <- fit_ttd(obs_conc = row$sf6_fmolperkg,
                        t_obs = row$year,
...
write.csv(results,"TTD_sf6_Glodap_labdata.csv", row.names = F)
dir()
